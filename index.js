...
client.login(process.env.BOT_TOKEN);`}, "express": { notes: [ "Use express.json() for JSON body parsing.", "Use process.env.PORT for deployment compatibility.", "Call app.listen only once." ], install: "npm install express", snippet:`import express from “express”;
...
});`}, "pg": { notes: [ "Use Pool from pg for PostgreSQL connections in Node.", "Prefer process.env.DATABASE_URL for connection configuration." ], install: "npm install pg", snippet:`import { Pool } from “pg”;

const pool = new Pool({
connectionString: process.env.DATABASE_URL
});

export default pool;`}, "drizzle-orm": { notes: [ "Do not guess Drizzle adapters or import paths.", "If exact setup is uncertain, prefer saying Unknown rather than fabricating APIs.", "Use reference-based generation when possible." ], install: "npm install drizzle-orm pg", snippet:`// Use only verified Drizzle setup.
// If exact Drizzle adapter or import path is uncertain, respond:
// “Unknown — requires verified reference.”`}, "typescript": { notes: [ "Do not assume Node runs .ts files directly without tsx, ts-node, or a build step.", "Prefer modern ESM-friendly tsconfig for Node 20+." ], install: "npm install -D typescript @types/node", snippet:`{
“compilerOptions”: {
“target”: “ES2022”,
“module”: “NodeNext”,
...
switch (type) {
case “discord”:
return `BOT_TOKEN= CLIENT_ID= GUILD_ID= GROQ_API_KEY= OWNER_ID=`;
...
// Auto-generate .env templates without using Claude/Groq
if (trimmedMessage.toLowerCase().includes(".env")) {
  ...
  await supabase.from("messages").insert({ session_id: sessionId, role: "user", content: trimmedMessage });
  await supabase.from("messages").insert({ session_id: sessionId, role: "assistant", content: template });
...
// Compress older context into summaries when chats get long
if (cleanHistory.length > 20) {
  const older = cleanHistory.slice(0, 10);
  const summaryText = await summarizeMessages(older);
  await supabase.from("summaries").insert({ session_id: sessionId, content: summaryText });
}

// Load latest summary and inject into model context
...
// FIX: default reply is now a sensible fallback instead of "Unknown — requires verified reference."
let reply = "Hey! What's up?";
let provider = "groq";
    ...
    try {
      const completion = await groq.chat.completions.create({
        ...
        messages: [
          ...
          ...(referenceContext
            ? [{ role: "system", content: `Reference snippets:\n${referenceContext}\n\nIf reference context and model memory differ, prefer the reference context.` }]
            : []),
          ...recentHistory,
